  ...
  i.audio.onloadedmetadata = () => {
    ...
    i.audio.ontimeupdate = () => {
      ...
      i.audio.addEventListener("ended", function () {
        // Найдите элемент, который вы хотите сделать целевым для прокрутки
        var targetElement = document.getElementById("afterWar");

        // Проверьте, найден ли элемент
        if (targetElement) {
          // Определите вертикальное смещение вверх, чтобы скролл был немного выше целевого элемента
          var offset = -300; // Измените это значение по вашему усмотрению

          // Получите координаты верхнего левого угла целевого элемента
          var rect = targetElement.getBoundingClientRect();
          var targetTop = rect.top + window.scrollY;

          // Прокрутите страницу к верхнему краю целевого элемента с учетом смещения
          window.scrollTo({
